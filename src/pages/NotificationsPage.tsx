  ...
  return (
    <div className="min-h-screen bg-app-bg text-white">
      <div className="flex items-center justify-between px-4 py-3">
        <button onClick={() => navigate(-1)} className="text-sm text-muted">← Back</button>
        <h1 className="text-lg font-semibold">Notifications</h1>
        <button onClick={markAllRead} className="text-sm text-muted hover:text-white">
          Mark all read
        </button>
      </div>
      ...
      <div className="px-4">
        {loading ? (
          <div className="space-y-2">{[...Array(5)].map((_, i) => (
            <div key={i} className="bg-input-fill rounded-lg h-14 animate-pulse" />
          ))}</div>
          ...
          notifications.map((n) => (
            <div
              ...
              className={`flex items-center gap-3 py-3 border-b border-chip-border/50 cursor-pointer ${!n.read ? 'bg-white/5 -mx-4 px-4' : ''}`}
            >
              {n.actor?.avatar_url && (
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm">
                  ...
                  <span className="text-muted">{typeLabels[n.type] || n.type}</span>
                </p>
