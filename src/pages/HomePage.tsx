  ...
  return (
    <div className="pb-20">
      ...
      <div className="px-4 pt-3 pb-2 flex items-center justify-between">
        <div>
          <h1 className="text-[22px] font-bold tracking-tight" style={{ fontFamily: "'Pacifico', cursive" }}>DriftScout</h1>
          <span className="text-[9px] text-muted">V1.0</span>
        </div>
      ...
      <div className="px-4 pb-3 flex items-center gap-3">
        <div className="flex-1 bg-input-fill rounded-2xl px-4 py-2.5 flex items-center gap-2 cursor-pointer" onClick={() => navigate('/locations')}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#888" strokeWidth="2">
            <circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/>
          </svg>
          <span className="text-[15px] text-muted">Search city or spot...</span>
        </div>
        <button
          onClick={handleRefresh}
          disabled={refreshing}
          className="bg-input-fill rounded-2xl px-4 py-2.5 text-sm font-medium text-white"
        >
          Filters
        </button>
      </div>
      ...
      <div className="px-4 py-2 flex items-center justify-center gap-1.5">
        ...
        <span className="text-[13px] text-muted">
          ...
          {filterTag && (
            <>
              {' '}<span>· #{filterTag}</span>
              <button onClick={() => setFilterTag(null)} className="ml-1 text-muted hover:text-white">✕</button>
            </>
          )}
        </span>
      </div>
      ...
      <div className="px-4">
        {loading ? (
          ...
          <>
            ...
            <div className="border-t border-tab-bar-border mt-4 pt-4">
              <Leaderboard />
            </div>
          </>
        )}
